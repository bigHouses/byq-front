  ...
  <div class="backtop">
    <div ref="imageWrapper">
      <!--<div style="width: 200px;">-->
       <!---->
      <!--</div>-->
      <!--<div class="formContainer" style="padding: 20px;margin-top: -78px;">-->
      <div class="formContainer" style="padding: 20px;">
        ...
        <div class="mb20" style="width: 1160px;height: 40px;">
          <el-upload style="float: right;margin-right: 60px;"
                     accept="txt"
            ...
            <el-button v-show="!isEditExist" class="ml100" type="primary">上传报告</el-button>
          </el-upload>
        </div>


        <div>
          <el-select @change="handleReportName" clearable filterable :disabled="readOnlyFlag" v-model="form.bianyaqxh" placeholder="请选择变压器序号">
            <el-option
          ...
          <el-select @change="handleXH" clearable class="ml100" filterable :disabled="readOnlyFlag" v-model="form.bianyqxh"
          ...
          <el-select @change="changeYear" clearable class="ml100" filterable :disabled="readOnlyFlag" v-model="form.year"
                     placeholder="请选择年份">
            <el-option
          ...
          <el-button v-show="form.reportType === '2'" style="margin-left: 30px;" type="primary" @click="downOrigin">下载报告原档</el-button>
      ...
      <div class="app-container formContainer" v-show="showForm">
        <!--试验资料-->
        <div v-if="form.reportType=='1'" id="formArea">
          ...
          <span class="flex-down" style="margin-top: 38px;">
      <el-input readonly value="              实             测               值Ukj(%)"
                class="border-t border-b border-r w300 flex-up"></el-input>
        <el-input readonly value="       A      " style="z-index: 999;" class="border-t border-b border-r w100"></el-input>
        <el-input readonly value="       B      " style="z-index: 999;" class="border-t border-b border-r w100"></el-input>
        <el-input readonly value="       C      " style="z-index: 999;" class="border-t border-b border-r border-r w100"></el-input>
      </span>
          ...
          <el-input :readonly="readOnlyFlag" v-model="form.frequencyList[0].frequency.gaopd"
                    class="border-t border-r w200 "></el-input>
          <el-dropdown v-if="!readOnlyFlag" style="margin-left: 10px;">
          ...
          <el-button v-if="!readOnlyFlag" style="margin-left:5px" :disabled="count1 !== 3" size="small"  type="primary" @click="getRxyData1">生成系数</el-button>
          ...
          <el-button v-if="!readOnlyFlag" style="margin-left:5px" :disabled="count2 !== 3" size="small"  type="primary" @click="getRxyData2">生成系数</el-button>
          ...
          <el-button v-if="!readOnlyFlag" style="margin-left:5px" :disabled="count3 !== 3" size="small"  type="primary" @click="getRxyData3">生成系数</el-button>
        ...
        <div v-if="form.reportType=='2'" >
          <div v-loading.fullscreen.lock="pdfLoading" >
            <iframe :src="filePath" width="100%" height="820" style="margin-top: 30px;"/>
          </div>
        </div>


        <div class="btns-submit">
          <el-button round v-if="!readOnlyFlag && !isEditExist" @click="saveDraft">保 存</el-button>
          <el-button type="primary" round @click="returnList">返回列表</el-button>
...
<script>
  import {getSelectParam, getPdf} from '@/api/table'
  import {saveDraft, getDraft, updateReport} from '@/api/form'
  ...
  export default {
    ...
    mounted: function () {
      if (this.$route.query.readOnlyFlag == undefined) {
        ...
        if(form.reportType === '2'){
          ...
          getPdf({reportId: this.form.id,reportType: 'pdf'}).then(response=>{
            this.filePath = window.URL.createObjectURL(response)
      ...
      if(this.form.reportType === '1'){
        this.initChart1DForDetail(0, 0)
        this.initChart1DForDetail(1, 1)
        this.initChart1DForDetail(2, 2)
        this.initChart2DForDetail(0, 3)
        this.initChart2DForDetail(1, 4)
        this.initChart2DForDetail(2, 5)
        this.initChart3DForDetail(0, 6)
        this.initChart3DForDetail(1, 7)
        this.initChart3DForDetail(2, 8)
        this.initChart1()
        this.initChart2()
        this.initChart3()
      }
    ...
    methods: {
      ...
      initChart1DForDetail(i, j) {
        this.option1.series[i] = {
          name: this.form.frequencyList[j].frequency.raozjfjfs,
          type: 'line',
          stack: '响应(dB)'
        }
        this.option1.legend.data[i] = this.form.frequencyList[j].frequency.raozjfjfs
        this.option1.xAxis.data = this.form.frequencyList[j].xlist
        this.option1.series[i].data = this.form.frequencyList[j].ylist
      },
      initChart2DForDetail(i, j) {
        this.option2.series[i] = {
          name: this.form.frequencyList[j].frequency.raozjfjfs,
          type: 'line',
          stack: '响应(dB)'
        }
        this.option2.legend.data[i] = this.form.frequencyList[j].frequency.raozjfjfs
        this.option2.xAxis.data = this.form.frequencyList[j].xlist
        this.option2.series[i].data = this.form.frequencyList[j].ylist
      },
      initChart3DForDetail(i, j) {
        this.option3.series[i] = {
          name: this.form.frequencyList[j].frequency.raozjfjfs,
          type: 'line',
          stack: '响应(dB)'
        }
        this.option3.legend.data[i] = this.form.frequencyList[j].frequency.raozjfjfs
        this.option3.xAxis.data = this.form.frequencyList[j].xlist
        this.option3.series[i].data = this.form.frequencyList[j].ylist
      },
      ...
      handleUploadData2(i, name, response){
        ...
        this.form.frequencyList[i].xlist = this.option2.xAxis.data
        this.form.frequencyList[i].ylist = this.option2.series[i].data
      },
      handleUploadData3(i, name, response){
        ...
        this.form.frequencyList[i].xlist = this.option3.xAxis.data
        this.form.frequencyList[i].ylist = this.option3.series[i].data
      },
      ...
      handleForm(form) {
        ...
        formCopy.frequencyList = [{},{},{},{},{},{},{},{},{}]
        for (let i = 0; i < formCopy.frequencies.length; i++) {
          formCopy.frequencyList[i].frequency = formCopy.frequencies[i]
          formCopy.frequencyList[i].xlist = JSON.parse(formCopy.frequencies[i].xlist)
          formCopy.frequencyList[i].ylist = JSON.parse(formCopy.frequencies[i].ylist)
        }
...
//        }
      ...
      downOrigin() {
        this.pdfLoading = true
        getPdf({reportId: this.form.id,reportType: 'word'}).then(response=>{
          let url = window.URL.createObjectURL(response);
          let wordPath = this.form.wordPath;
          this.downloadFile(url, wordPath.substring(wordPath.lastIndexOf('/')+1))
          this.pdfLoading = false
        })
      },
      ...
      submitForm() {
        ...
        this.form.userId = JSON.parse(getUser()).id
        //updateReport
      ...
      returnList() {
        ...
        this.$router.push({
          name: '试验报告查询',
          query: {condition: this.$route.query.condition}
        })
      }
    }
  }
