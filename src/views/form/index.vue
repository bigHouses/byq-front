  ...
  <div>
    <div ref="imageWrapper">
      <div class="formContainer" style="padding: 20px">
        ...
        <div>
          <el-select filterable @change="handleReportName" clearable filterable :disabled="readOnlyFlag" v-model="form.bianyaqxh"
          ...
          <el-select filterable @change="handleXH" clearable class="ml100" filterable :disabled="readOnlyFlag"
          ...
          <el-upload style="display: initial;"
                     accept="txt"
                     :action="host + '/transformer/readFile/uploadFile'"
                     :before-upload="beforeUploadReport"
                     :on-success="uploadReport"
                     name="multipartFile"
                     :headers="uploadHeader">
            <el-button v-show="!isEditExist && uploadReportEnable" style="margin-left: 5px;" type="primary">上传报告</el-button>
          </el-upload>
        </div>
      </div>
...
<script>
  ...
  import {saveDraft, getDraft, generateData} from '@/api/form'
  ...
  export default {
    ...
    methods: {
      ...
      beforeUploadReport(file) {
        ...
        if (ext !== 'doc' && ext !== 'docx') {
          this.$message('仅支持上传doc或docx格式报告!', 4000)
          return false
        }
        if (this.form.city === ''
          || this.form.biandz === ''
      ...
      uploadReport(response, file, fileList) {
        if (200 === response.code) {
          this.form.reportPath = response.data
          this.submitForm()
        } else {
          this.$message('上传报告失败,请重试')
        }
      ...
      checkShowForm() {
        ...
        this.showForm = true
        this.uploadReportEnable = false
      },
      submitForm() {
        if ('2' !== this.form.reportType) {
